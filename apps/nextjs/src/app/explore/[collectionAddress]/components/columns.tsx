...
export const columns: ColumnDef<Token>[] = [
  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      const tokenOwner = row.getValue("owner");
      const owner =
  ...
  },
  {
    ...
    cell: ({ row }) => {
      const topBid = row.getValue("top_bid");

      if (topBid?.amount === undefined || topBid.amount === null) {
        return <div className="w-[100px] items-center">-</div>;
      }
      return (
        <div className="flex w-[100px] items-center">
  ...
  },
  {
    ...
    cell: ({ row }) => {
      const listedTimeStamp = row.getValue("listed_timestamp");
